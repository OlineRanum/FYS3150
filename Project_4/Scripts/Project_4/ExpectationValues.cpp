...
void ExpectationValues::Estimate_ExpectationValues(double* Energy, double* Magnetization, int N_mc, double T, double* values){
    double k_b = 1;
    double beta = 1/(k_b*T);
    double expval_E = 0, expval_E2  = 0, expval_M = 0, expval_M2 = 0, expval_abs_M = 0, boltzman_sum = 0, bzf;
    ...
    for (int i = 0; i < N_mc; i++){
        ...
        expval_M2 += fabs(Magnetization[i]); }

    values[0] = expval_E/N_mc;
    values[1] = expval_E2/N_mc;
    values[2] = expval_M/N_mc;
    values[3] = expval_M2/N_mc;
    values[4] = expval_abs_M/N_mc;
