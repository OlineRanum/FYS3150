...
\subsubsection{Quantum harmonic osccilator with two electrons}
Now we will study the same harmonic osccilator, but with two electrons. This means the electrons will also interact via a repulsive Coloumb interaction. We start by looking at the single-electron equation
\begin{equation*}
    -\frac{\hbar^2}{2m}\frac{d^2}{dr^2}u(r)+\frac{1}{2}kr^2u(r) = E^{(1)}u(r),
\end{equation*}
here $E^{(1)}$ stands for the energy with only one electron. For two electrons, with no repulsive Coloumb interaction for now, we get this Schroedinger equation
\begin{equation*}
...
\end{equation*}
Now  we are dealing with both a two-electron wave function $u(r_1,r_2)$ and two electron energy $E^{(2)}$.\par
Since we have no interaction we have a solution on closed form, that means it can be written as the product of two single-electron wave functions.\par
We introduce two new coordinates, the relative coordinate $\bm{r}=\bm{r}_1-\bm{r}_2$ and the center-of-mass coordinate $\bm{R}=1/2(\bm{r}_1+\bm{r}_2)$. And now the radial Schroedinger equation reads
\begin{equation*}
    \left(-\frac{\hbar^2}{m}\frac{d^2}{dr^2}-\frac{\hbar^2}{4m}\frac{d^2}{dR^2}+\frac{1}{4}kr^2+kR^2\right)u(r,R) = E^{(2)}u(r,R).
\end{equation*}
\par We can separate the equations for $r$ and $R$ via the ansatz for the wave function $u(r,R)=\psi(r)\phi(R)$ and the energy is given by the sum of the relative energy and the center-of-mass energy, so we rewrite it like
\begin{equation*}
    E^{(2)} = E_r+E_R.
\end{equation*}
\par To include the repulsive Coulomb interaction between two electrons we add the term
\begin{equation*}
...
\end{equation*}
This equation is similar to the one we had previously when we had just one electron, and again we introduce
a dimensionless variable $\rho = r/\alpha$. By repeating the same
...
\end{equation*}
We want to make the equation as similar to that in \ref{qho}
...
For one selection of $\rho_{max}$ it is possible to further investigate the trends in the relative error as a function of N. We did this for $\rho_{max} = 5$, and in figure \ref{fig:RE}.a we see a decay in the relative error with increasing N as previously indicated. This trend is not surprising, as one would typically expect a Toplitz matrix to have a relative error going like $\order{h^2}$. The function form of an error going as $\order{h^2}$ and our error-estimate appears to be the same. \\ \indent 
